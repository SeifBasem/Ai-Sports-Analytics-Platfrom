...
try:
    ...
    print("Warning: timm not installed. MultiDimStacker may fail if initialized.")
    # Create dummies so the file at least parses
    DropPath = None
    create_conv2d = None
    get_act_layer = None
    get_norm_act_layer = None
...
class InvertedResidual3d(nn.Module):
    def __init__(self,
        ...
        if drop_path_rate > 0 and DropPath is not None:
...
class MultiDimStacker(nn.Module):
    def __init__(self,
                 ...
                 num_3d_stack_proj: int = None,
                 expansion_3d_ratio: int = 3,
        ...
        if(num_3d_stack_proj is None):
            num_3d_stack_proj = self.num_features//num_frames

        self.drop_rate = drop_rate

        act_layer_fn = get_act_layer(act_layer)
        norm_act_layer = get_norm_act_layer(nn.BatchNorm2d, act_layer_fn)
    ...
    def forward_2d(self, x):
        x = x.contiguous()
        B, N, COL_CHANS, H, W = x.shape

        num_stacks = N // self.stack_size

        if(COL_CHANS==1):
            x = x.view(B * num_stacks, self.stack_size, H, W)
        else:
            assert COL_CHANS==3
            x = x.view(B * N, COL_CHANS, H, W)

        x = self.conv2d_encoder(x)[-1]
        x = self.conv2d_projection(x).contiguous()
        _, _, h_down, w_down = x.shape

        if(COL_CHANS==1):
            x = x.view(B, num_stacks, self.num_3d_features, h_down, w_down)
        else:
            x = x.view(B, N, self.num_3d_features, h_down, w_down)

        return x

    def forward_3d(self, x):
        b, t, c, h, w = x.shape
        assert c == self.num_3d_features
        x = x.transpose(1, 2)
        x = self.conv3d_encoder(x)
        x = x.transpose(1, 2)
        x = x.reshape(b * t, c, h, w)
        x = self.conv3d_projection(x)

        x = x.view(b, self.num_features, h, w)
        return x
